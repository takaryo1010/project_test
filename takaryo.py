...
player=Player("sample",100,5,3)#テスト用プレイヤー宣言
...
class Grapher():
    ...
    def append_sleepiness(self,count,value_by_level,screen,screen_size):
        global flag_sleepiness
        if flag_sleepiness==0:
            i=random.randint(int(value_by_level*0.7),value_by_level*1.5)
        ...
        if i==count:
            ...
            x=map_choiced[0]
            y=map_choiced[1]
            self.list_sleepiness.append(Sleepiness(x,y,value_by_level/15,int(value_by_level/30),random.randint(1,3)))
            
            flag_sleepiness=0
        for i in self.list_sleepiness:
            screen.blit(sleepiness_image,(i.x,i.y))
    def move_sleepiness(self,screen_size):
        global dx,dy,rect_player,amplitude_value
        ...
        for i in self.list_sleepiness:
            count+=1
            center_x=int(rect_player[0]-i.x)
            center_y=int(rect_player[1]-i.y)
            ...
            if center_x>0:
                i.x+=math.cos(θ)*i.speed+18**(-2.1*math.radians(amplitude_value))*math.cos(5*math.radians(amplitude_value))
                i.y+=math.sin(θ)*i.speed+18**(-2.1*math.radians(amplitude_value))*math.cos(5*math.radians(amplitude_value))
            if center_x<0:
                i.x-=math.cos(θ)*i.speed+18**(-2.1*math.radians(amplitude_value))*math.cos(5*math.radians(amplitude_value))
                i.y-=math.sin(θ)*i.speed+18**(-2.1*math.radians(amplitude_value))*math.cos(5*math.radians(amplitude_value))
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if pressed_key[K_SPACE]:
                return cursor                 # SPACEボタンが押されたらループ解除
        pygame.display.update() 
        
def decide_player(res):
        if res=="0":
            return Player("sample",100,5,5)
...
def move_player(player):
    ...
    for event in pygame.event.get():
            ...
            # キーを押したとき 
            pressed_key = pygame.key.get_pressed()
            if pressed_key[K_LEFT]:#移動系
                #dx+=player.speed  
                rect_player.move_ip(-player.speed,0)
                
            if pressed_key[K_RIGHT]:#移動系
                rect_player.move_ip(player.speed,0)
                #dx-=player.speed
            if pressed_key[K_UP]:#移動系
                rect_player.move_ip(0,-player.speed)
                #dy+=player.speed
            if pressed_key[K_DOWN]:#移動系
                rect_player.move_ip(0,player.speed)
                #dy-=player.speed
...
#-------------------------------------------------------------------------------
#-----------画像----------------------------------------------------------------
player_image=pygame.image.load("画像/sleepiness.jpg")
...
sleepiness_image=pygame.image.load("画像/sleepiness.jpg")
