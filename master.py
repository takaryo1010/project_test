...
class Field:#プレイ画面のクラス意義
    ...
    def append_b(self, x, y, dx, dy):#弾をリストに追加する関数
    ...
    def move_bullet(self, screen,level):#弾の動きを管理する関数
        for c_b, bullet in enumerate(self.bullets):
            ...
            for c_e, enemy in enumerate(self.enemys):
                ...
                if leng < (self.player.r + enemy.r) / 2:
                    self.enemys.pop(c_e)
    ...
    def append_sleep(self):#敵をリストに追加する関数
        ...
        if enemy_choice==0:
            self.enemys.append(Enemy(x, y,r=80, hp=100, attack=50, speed=1,various=0))
        elif enemy_choice==1:
            self.enemys.append(Enemy(x, y,r=60, hp=10, attack=10, speed=2.5, various=1))
        elif enemy_choice==2:
